...
class _SpotDetailScreenState extends ConsumerState<SpotDetailScreen> {
  ...
  Future<void> _loadAlternatives() async {
    try {
      final response = await ref
          ...
          .dio
          .get('/spots/${widget.spot.slug}/alternatives');
      if (mounted) {
        setState(() => alternatives = (response.data['data'] as List)
  ...
  Future<void> _directions() async {
    ...
    await launchDirections(widget.spot);
  }
          ...
          if (spot.crowdStatus == 'estimated_busy')
            Card(
                ...
                child: const Padding(
                    ...
                    child: Text(
                        'This destination may be busy based on recent JuanderQuest activity—not live occupancy. Try a similar place below.'))),
          Wrap(
              spacing: 8,
              children: spot.tags
                  .map((tag) => Chip(label: Text(tag.replaceAll('_', ' '))))
                  .toList()),
          ...
          if (spot.questId != null)
            Padding(
                ...
                child: FilledButton.tonalIcon(
                    ...
                    label: const Text('Play this quest'))),
          const SizedBox(height: 24),
          Text('Similar places nearby',
              style: Theme.of(context)
                  .textTheme
                  .titleLarge
                  ?.copyWith(fontWeight: FontWeight.bold)),
          const Text('Reviewed alternatives within 10–25 km.'),
          const SizedBox(height: 8),
          if (loadingAlternatives)
            const LinearProgressIndicator()
          else if (alternatives.isEmpty)
            const Text('No credible similar alternatives are available yet.')
          else
            ...alternatives.map((item) => Card(
                child: ListTile(
                    title: Text(item.name),
                    subtitle: Text(
                        '${item.distanceKm?.toStringAsFixed(1) ?? '?'} km · ${item.reasons.take(2).join(' · ')}'),
                    onTap: () =>
                        context.push('/explore/${item.slug}', extra: item)))),
          const SizedBox(height: 16),
          Text(
              'Source: ${spot.sourceName} · Crowd status is an app-activity estimate.',
              style: Theme.of(context).textTheme.bodySmall),
        ]));
  }
}

Future<void> launchDirections(SpotModel spot) => launchUrl(
    Uri.parse(
